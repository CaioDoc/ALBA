...
export default function AdminCursosPage() {
  ...
  useEffect(() => {
    const savedCourses = localStorage.getItem('alba_cursos_v19');
    if (savedCourses) {
      setCourses(JSON.parse(savedCourses));
    } else {
      setCourses(scrapedCourses);
      localStorage.setItem('alba_cursos_v19', JSON.stringify(scrapedCourses));
    }
  }, []);
  ...
  const handleDelete = (id: number) => {
    if(confirm('Tem certeza que deseja deletar ou suspender este curso permanentemente?')) {
      ...
      localStorage.setItem('alba_cursos_v19', JSON.stringify(newCourses));
    }
  };
